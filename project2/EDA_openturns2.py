...
class HvEDA:
    ...
    def Make_X_UsingCopula2(self):
        Xvar = range(30)
        collection = CopulaCollection(m)
        #factory1 = FarlieGumbelMorgensternCopulaFactory()
      #  factory1 = FrankCopulaFactory()
        factory1 = NormalCopulaFactory()
       # factory1 = AliMikhailHaqCopulaFactory()
        factory2 = ClaytonCopulaFactory()
        Xvar = []
#        Varrtmp = self.Choice()
        varr = list(zip(*self.ListNonDominated))
        xx = rd.sample( self.ListNonDominated , len(self.ListNonDominated)/2) # demi de meilleur sol random
        ...
        for j in range(m):
            ...
            snd = NumericalSample(0,2)  # create a ensemble to enter nondimanated set0 
            for i in range(len(ym)):
                ...
                snd.add(point1)
            if j % 2 == 0:
                collection[j] = factory2.build(snd) #create a clayton copula
            else:
                collection[j] = factory1.build(snd)
        copula = ComposedCopula(collection)
        list_new = copula.getSample( 100 )
        Zvar = []
        for i in range(100):
...
hv = HyperVolume(referencePoint)      
volume = []
test = HvEDA(100,FonctionObject03)
test.Evaluate()
...
fileData = open("hypervolume_before.txt","w")            
i = 0
while i < len( volume ):
