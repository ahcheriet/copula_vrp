...
#            if ( any(map(lambda x:True if (x < -5 or x >5) else False ,Zvar[index][1:])) == True ) or (Zvar[index][0]<0 or Zvar[index][0] > 1):     # pour f4
    ...
    def Make_First_Pop(self):
        ligne = []
        i = 1
        ligne.append((np.random.uniform(0,1,100)).tolist())
        while i < m:
...
#        front = self.ApplyFunctions(self.ListNonDominated)
 #       volume1 = hv.compute(front)
        ...
        self.ListNonDominated , self.front_0 = self.Load_ListND(self.ListNonDominated, 30)
        #self.filtrate()
     #   self.ListNonDominated = self.Load_ListND(self.ListNonDominated, 30)
#        if ( len(self.ListNonDominated) > 100 ):
#            self.ListNonDominated = rd.sample( self.ListNonDominated , 70)
